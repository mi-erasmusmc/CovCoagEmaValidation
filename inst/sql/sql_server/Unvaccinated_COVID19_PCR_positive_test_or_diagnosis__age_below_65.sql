...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (3661405,3655976,3661748,3661406,3662381,756031,3656667,3656668,37311061,3656669,37310284,3661885,37310283,37310286,3663281,3661631,37310287,37310254,704995,700297,704996,700296,3661408,756039,3655977,3655975,3661632)

) I
) C UNION ALL 
SELECT 3 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (37003518,37003436,724905,35891709,35891522)
...
WHERE (C.measurement_date >= DATEFROMPARTS(2020, 9, 1) and C.measurement_date <= DATEFROMPARTS(2021, 3, 1))
AND C.value_as_concept_id in (9191,4126681,4181412,45879438,45884084,45877985)
...
WHERE (C.condition_start_date >= DATEFROMPARTS(2020, 9, 1) and C.condition_start_date <= DATEFROMPARTS(2021, 3, 1))
...
WHERE (C.observation_date >= DATEFROMPARTS(2020, 9, 1) and C.observation_date <= DATEFROMPARTS(2021, 3, 1))
...
WHERE C.value_as_concept_id in (45884084,9191,45879438,4181412,4126681,45877985)
-- End Measurement Criteria
